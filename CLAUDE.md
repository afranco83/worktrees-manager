...
**Estado actual: Fase 6 — Estado de cambios sin commitear, cerrada y mergeada en `main`** ([PR #6](https://github.com/afranco83/worktrees-manager/pull/6), `docs/ROADMAP.md`). Próxima fase: **Fase 7 — Integración con Pull Requests** (ver estado de la PR asociada a un worktree sin salir del dashboard). Fases 0-6 cerradas: documentación, scaffolding, modelo de datos, gestión de proyectos, ciclo de vida de worktrees, arranque/parada de entornos de dev con logs en tiempo real (con detección de puertos en monorepos, comando de arranque por worktree, copia automática de `.env` y comando posterior a la creación añadidos como pulido posterior al cierre de la Fase 5) y aviso de seguridad ante el borrado de un worktree (cambios sin commitear / commits sin subir a ningún remoto conocido, Fase 6 — ver `docs/ROADMAP.md` para el detalle y los ADR-0007 a ADR-0012).
...
- No adelantes trabajo de fases futuras (p. ej. no montes la integración con Pull Requests de la Fase 7 mientras no se pida explícitamente) salvo que el usuario lo pida explícitamente.
