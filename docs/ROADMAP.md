...
## Tarea — Endurecimiento de seguridad del repositorio _(cerrada — 2026-08-17)_

**Objetivo**: con el paquete ya publicado en el registro público de npm, reducir la superficie de riesgo del repositorio en GitHub (cuenta gratuita, sin GitHub Advanced Security de pago). No es continuación de la Fase 9 — trabajo nuevo, sin fase asignada, documentado como tarea propia según el criterio fijado al cerrar dicha fase.

**Hecho**:

- [x] **Ruleset `protect-main`** (`Settings → Rules`, API de rulesets — no la branch protection clásica, que en cuenta gratuita de repo público tiene menos granularidad): bloquea push directo, force-push y borrado de `main`; exige PR + el check `ci` (lint+typecheck+test) en verde y la rama actualizada con `main` antes de mergear. **Se aplica también al propio owner/admin** (`current_user_can_bypass: never`) — decisión explícita del usuario: en un repo de un único colaborador, si el admin puede saltársela siempre, la regla no protege nada de verdad. Verificado con un push directo real a `main` (rechazado con `GH013`) y con una PR de prueba completa (rama → PR #12 → check `ci` en verde → merge — nada de esto era posible antes sin la regla).
- [x] **Dependabot**: alertas de vulnerabilidades + actualizaciones de seguridad automáticas activadas (estaban desactivadas por defecto). Secret scanning + push protection ya estaban activos de fábrica en un repo público, sin acción necesaria.
- [x] **Limpieza de las 27 alertas que destapó Dependabot al activarse** (23 abiertas + 4 ya auto-descartadas por GitHub): revisadas una a una con `pnpm why` para diferenciar impacto real de ruido, antes de tocar nada.
  - **`react-router` 8.2.0→8.3.0** ([PR #11](https://github.com/afranco83/worktrees-manager/pull/11), de Dependabot): bypass de CSRF en modo RSC de `react-router`, que esta SPA no usa (`createBrowserRouter`), pero el paquete va igualmente en el JS publicado. El lockfile que generó el propio bot violaba la política `minimumReleaseAge` de pnpm 11 (arrastraba un `electron-to-chromium` publicado hacía menos de 24h) y tumbaba el check `ci` — se corrigió re-resolviendo el lockfile a mano (`pnpm clean --lockfile && pnpm install`) sobre la misma rama del bot.
  - Esa misma re-resolución completa del lockfile subió de rebote `fastify`/`socket.io` a sus últimas patch, parcheando de paso `find-my-way`, `fast-uri` y `socket.io-parser` (alertas altas, dependencias directas reales de `apps/server`) — verificado que ya no hacía falta una PR separada para esto antes de crear la rama.
  - **`shadcn` movido de `dependencies` a `devDependencies`** en `apps/dashboard/package.json` ([PR #13](https://github.com/afranco83/worktrees-manager/pull/13)): es el CLI de scaffolding de shadcn/ui (`pnpm dlx shadcn add ...`), nunca se importa en código de la app. Estaba mal clasificado desde que se instaló en la Fase 3, y arrastraba como alertas "runtime" toda su cadena transitiva (`hono`, `ip-address`, `@modelcontextprotocol/sdk`, `dotenvx`...) — código que la app nunca ejecuta. Resto de alertas de ruido (`postcss`, `nanoid`, `undici` vía `jsdom`, `js-yaml`/`brace-expansion` vía ESLint/commitlint) resueltas de paso por ser la misma cadena o herramientas de solo build/dev, que además no viajan en el paquete publicado (`apps/server` no incluye `node_modules` del dashboard).
